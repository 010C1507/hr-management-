...
- **ตั้งค่าความสัมพันธ์เอง** — ในโปรไฟล์แต่ละคนมีแท็บ "ความสัมพันธ์" ให้เลือกพ่อ/แม่/คู่สมรสจาก dropdown และเพิ่ม-ลบลูก/พี่น้องได้ **การเปลี่ยนแปลงมีผลสองทางทันที** (เลือกน้องสาว → ฝั่งเธอเห็นคุณเป็นพี่ชาย, เลือกพ่อ → ฝั่งพ่อเห็นคุณเป็นลูก) เพราะเก็บเป็นเส้นเชื่อมจริง `father_id`/`mother_id`/`spouse_id` เส้นเดียวที่ทั้งสองฝ่ายใช้ร่วมกัน
...
4. คัดลอกเนื้อหาทั้งหมดจากไฟล์ [`supabase/schema.sql`](./supabase/schema.sql) ในโปรเจกต์นี้ วางแล้วกด **Run** — จะได้ตาราง `family_members`, `growth_records`, `insurance_policies`, `welfare_benefits`, `education_history`, `medical_records` พร้อมข้อมูลตัวอย่าง
   ...
   - หากต้องการตั้งค่าความสัมพันธ์เอง (พ่อ/แม่/คู่สมรส/ลูก/พี่น้อง) ให้รัน [`supabase/migrations/008_relationship_edges.sql`](./supabase/migrations/008_relationship_edges.sql) — เพิ่มคอลัมน์ `father_id`/`mother_id`/`spouse_id` และแปลงข้อมูลเดิมให้เป็นเส้นเชื่อมอัตโนมัติ
